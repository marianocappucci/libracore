...
from . import get_config, _npm_api
...
def _docker_build_ssh_args(repo_root: Path) -> list[str]:
    """Arma los `--ssh id=key` para `docker build`. Se pasan `default` y
    `libracore` apuntando a la misma key (compat con Dockerfile viejos de
    un solo mount id y con los nuevos que ya usan `id=libracore`
    explícito); `libracommerce` solo si el producto depende de ese paquete
    (si no, la key puede ni existir en esta máquina). Pasar un `--ssh` de
    más para un id que el Dockerfile no monta es inofensivo (BuildKit lo
    ignora), pero pasar uno con una key inexistente rompe el build."""
    args = [
...
def cmd_actualizar(slugs: list[str] | None = None):
    ...
    r = subprocess.run(
        ["docker", "build", *_docker_build_ssh_args(cfg.repo_root), "-t", cfg.image_name, "."],
